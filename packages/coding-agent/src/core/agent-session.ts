...
export class AgentSession {
	...
	private _installAgentNextTurnRefresh(): void {
		...
		this.agent.prepareNextTurnWithContext = async (turn, signal) => {
			...
			// LLM-visible half of the Timeline: the declaration convention plus the
			// recent failed/partial decisions. Empty when tracking is off and nothing
			// failed, so the prompt stays byte-for-byte identical to upstream.
			Object.assign(options.sections, this._schemaDecisionPromptSections());
	...
	/**
	 * Schema-decision content for the system prompt, as named sections. The prompt
	 * renderer wraps every non-preamble section in a tag of the same name, so the
	 * values here are untagged bodies. `schema_declaration_convention` states the
	 * <plan>/<expected> contract; `recent_decisions` carries batches where the plan
	 * did not match reality. Returns {} when tracking is disabled.
	 */
	private _schemaDecisionPromptSections(): Record<string, string> {
		if (!isSchemaDecisionTrackingEnabled()) return {};
		const sections: Record<string, string> = {
			schema_declaration_convention: SCHEMA_DECLARATION_CONVENTION,
		};
		const digest = this.sessionManager.getRecentDecisionsDigestBody(5);
		if (digest) sections.recent_decisions = digest;
		return sections;
	}
	...
	private async _handlePostAgentRun(): Promise<boolean> {
		...
		if (
			...
			const textBlock = message.content.find((c): c is TextContent => c.type === "text");
	...
	async prompt(text: string, options?: PromptOptions): Promise<void> {
		...
		try {
			...
			// Same sections on a run's first request: the declaration contract is then in
			// effect immediately and the sections do not flap between runs.
			Object.assign(result.systemPromptOptions.sections, this._schemaDecisionPromptSections());
