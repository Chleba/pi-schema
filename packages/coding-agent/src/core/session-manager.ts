...
export class SessionManager {
	...
	/**
	 * Digest body without the `<recent_decisions>` tag, for injection as a named
	 * system-prompt section (the prompt renderer supplies the tag).
	 */
	getRecentDecisionsDigestBody(limit = 5): string {
	 ...
	 * (the LLM-visible half of the Timeline), wrapped in `<recent_decisions>`. Only
	 * failed or partial decisions are surfaced — successes add noise and offer no
	 * revision value. Fewer than `limit` entries are returned so sessions without
	 * failures stay quiet.
	 */
	getRecentDecisionsDigest(limit = 5): string {
		const body = this.getRecentDecisionsDigestBody(limit);
		return body ? `<recent_decisions>\n${body}\n</recent_decisions>` : "";
	}
